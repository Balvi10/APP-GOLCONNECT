...
function PhysBox({ icon, value, label }) {
  return (
...
function SeasonRow({ label, value, last }) {
  return (
...
function ContactField({ icon, label, value: init, onSave }) {
...
async function persistirUser(userActual, setUser, perfilNuevo) {
  const userActualizado = { ...userActual, perfilJugador: perfilNuevo };
  ...
  const guardarFisico = async () => {
    ...
    try {
      await apiJugador.actualizarPerfil({
        altura_cm:             editAltura     ? parseInt(editAltura)     : undefined,
        peso_kg:               editPeso       ? parseInt(editPeso)       : undefined,
        pierna_habil:          editPierna     || undefined,
        posicion_principal:    editPosP       || undefined,
        posicion_secundaria:   editPosS       || undefined,
        temporada_partidos:    editPartidos   ? parseInt(editPartidos)   : undefined,
        temporada_goles:       editGoles      ? parseInt(editGoles)      : undefined,
        temporada_asistencias: editAsistencias ? parseInt(editAsistencias) : undefined,
      });

      const perfilNuevo = {
        ...(perfil ?? {}),
        altura_cm:             editAltura     ? parseInt(editAltura)     : perfil?.altura_cm,
        peso_kg:               editPeso       ? parseInt(editPeso)       : perfil?.peso_kg,
        pierna_habil:          editPierna     || perfil?.pierna_habil,
        posicion_principal:    editPosP       || perfil?.posicion_principal,
        posicion_secundaria:   editPosS       || perfil?.posicion_secundaria,
        temporada_partidos:    editPartidos   ? parseInt(editPartidos)   : perfil?.temporada_partidos,
        temporada_goles:       editGoles      ? parseInt(editGoles)      : perfil?.temporada_goles,
        temporada_asistencias: editAsistencias ? parseInt(editAsistencias) : perfil?.temporada_asistencias,
      };
        ...
        <View style={styles.topNavRight}>
          ...
          <Pressable onPress={() => setLogoutModal(true)} style={styles.avatarCircle}>
            {fotoLocal
              ...
              : <AvatarImage fotoPath={p.foto_perfil} apiBaseUrl={API_BASE_URL} size={36} iconSize={22} style={{ borderWidth: 0 }} />}
          </Pressable>
        </View>
      </SafeAreaView>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom:100 }}>
        ...
        <View style={[styles.px, styles.section]}>
          <View style={styles.glassCard}>
            ...
            {[['Spain',45],['England',20],['Italy',15],['Germany',10]].map(([c,pt]) => (
