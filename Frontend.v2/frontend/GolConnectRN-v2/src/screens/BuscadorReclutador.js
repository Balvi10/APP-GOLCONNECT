...
// ── Card jugador ──────────────────────────────────────────────────────────
function JugadorCard({ perfil, onVerPerfil }) {
  const u      = perfil.user ?? {};
  ...
  const estadoColor = {
    ...
    amateur:     '#FFC400',
    profesional: '#4FC3F7',
  }[perfil.estado ?? 'libre'] ?? colors.onSurfaceVariant;
...
export default function BuscadorReclutador() {
  ...
  useEffect(() => {
    setLoading(true);
    apiRec.buscar({})
      .then(data => setLista(data?.data ?? data ?? []))
      .catch(() => {})
...
const styles = StyleSheet.create({
  ...
  cardAvatar:    { width: 60, height: 60, borderRadius: 30, backgroundColor: colors.surfaceContainerHighest, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.outlineVariant, overflow: 'hidden' },
