...
// ── Item de trayectoria ───────────────────────────────────────────────────
function TrayItem({ club, division, partidos, inicio, fin, last }) {
  return (
...
export default function DetalleCandidato() {
  ...
  const trayec   = perfil.trayectoria ?? [];
  ...
  const golsPP = perfil.temporada_partidos > 0
    ? (perfil.temporada_goles / perfil.temporada_partidos).toFixed(2)
    : '0.00';
  const efectividad = perfil.temporada_goles > 0
    ? Math.min(Math.round((perfil.temporada_goles / (perfil.temporada_partidos || 1)) * 100), 100)
    : 0;
...
const styles = StyleSheet.create({
  ...
  avatar:       { width: 88, height: 88, borderRadius: 12, borderWidth: 2, borderColor: colors.primaryFixed },
  ...
  reportAvatar: { width: 32, height: 32, borderRadius: 16, backgroundColor: colors.secondaryContainer },
