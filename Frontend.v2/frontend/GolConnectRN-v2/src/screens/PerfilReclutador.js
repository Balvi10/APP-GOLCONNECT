...
import {
  ...
  ScrollView, ActivityIndicator, Linking, Alert, Modal as RNModal,
} from 'react-native';
...
// ── Card credencial ────────────────────────────────────────────────────────
function CredCard({ titulo, subtitulo, icon, color, onVer }) {
  return (
...
// ── Card contacto de red ──────────────────────────────────────────────────
function ContactoCard({ nombre, club }) {
  return (
...
export default function PerfilReclutador() {
  ...
  const [verDocModal, setVerDocModal] = useState(null); // { titulo, descripcion }
  ...
  // Recargar cada vez que la pantalla obtiene foco (ej: al volver de Editar Perfil)
  useFocusEffect(
    useCallback(() => {
      cargar();
    }, [cargar])
  ...
  // También leer del contexto si está disponible (tras editar perfil)
          ...
          <View style={styles.glassCard}>
            ...
            {credenciales.map((c, i) => (
              <CredCard
                ...
                icon="description"
                color={colors.primaryFixed}
                onVer={() => setVerDocModal({ titulo: c.titulo, descripcion: c.estado ?? 'Sin descripción' })}
              />
            ))}
          </View>
...
const styles = StyleSheet.create({
  ...
  avatarWrap:      { },
  avatar:          { width: 96, height: 96, borderRadius: 12, borderWidth: 3, borderColor: colors.background, backgroundColor: colors.surfaceContainerHighest },
