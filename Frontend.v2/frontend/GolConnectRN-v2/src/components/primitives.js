...
export function OutlineButton({ label, onPress, icon, style, textStyle, color = colors.primary }) {
  return (
    <Pressable
      onPress={onPress}
      style={({ pressed }) => [
  ...
  return (
    <View style={[{ gap: 6, marginBottom: 16 }, style]}>
      ...
      <View style={styles.fieldWrap}>
        ...
        <TextInput
          style={[styles.input, leftIcon && { paddingLeft: 8 }]}
          value={value}
...
export function Badge({ label, bg = colors.primaryFixed, color = colors.onPrimaryFixed, style, textStyle }) {
  return (
