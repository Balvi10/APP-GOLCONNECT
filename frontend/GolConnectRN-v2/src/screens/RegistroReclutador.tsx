...
function Picker({ label, value, options, onSelect }: PickerProps) {
...
export default function RegistroReclutador() {
  ...
  // Credencial
  const [archivoCredencial, setArchivoCredencial] = useState<DocumentPicker.DocumentPickerAsset | null>(null);
  ...
  const handleRegistro = async () => {
    ...
    setLoading(true);
    try {
      const data = await auth.registrarReclutador(payload as any);
      setToken(data.token);
      setUser(data.user);
      ...
      <View style={styles.body}>
        ...
        <Card style={{ marginTop: 24, borderColor: 'rgba(195,244,0,0.2)' }}>
          ...
          <Pressable
            ...
            style={[styles.uploadBtn, archivoCredencial ? { borderColor: colors.primaryFixed } : null]}
          >
            <Icon
