...
import { avisos as apiAvisos, jugadores as apiJugadores } from '../api';
...
function CandidatoCard({ candidato, seleccionado, onSeleccionar, onVerPerfil }: CandidatoCardProps) {
  const jugador = candidato.jugador ?? ({} as any);
    ...
    <Pressable style={styles.card} onPress={onVerPerfil}>
      ...
      <View style={styles.cardBottom}>
        ...
        <Pressable
          ...
          onPress={(e: GestureResponderEvent) => { e.stopPropagation?.(); onSeleccionar(); }}
        >
...
export default function CandidatosActivos() {
  ...
  const route = useRoute<RouteProp<RootStackParamList, 'candidatos-activos'>>();
  ...
  const toggleSeleccionar = (c: Postulacion) => {
    const id = c.id;
...
const styles = StyleSheet.create({
  ...
  cardGrad: {
    position: 'absolute', bottom: 0, left: 0, right: 0, height: 200, backgroundColor: 'transparent',
    // gradiente manual
