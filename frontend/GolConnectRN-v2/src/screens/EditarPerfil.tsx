...
import type { CredencialReclutador } from '../types';
...
function ExpandSection({ icon, title, children, defaultOpen = false }: ExpandSectionProps) {
...
function FormField({ label, value, onChangeText, placeholder, keyboardType, secureTextEntry }: FormFieldProps) {
  return (
...
export default function EditarPerfil() {
  ...
  const [fotoServidor, setFotoServidor] = useState<string | null>(null); // foto ya guardada en el servidor
  const [credencialSelec, setCredencialSelec] = useState<DocumentPicker.DocumentPickerAsset | null>(null);
  const [credenciales, setCredenciales] = useState<CredencialReclutador[]>([]);
  ...
  useEffect(() => {
    apiRec.perfil()
      .then((data) => {
        const p = data?.perfilReclutador ?? ({} as any);
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, paddingBottom: 60 }}>
        ...
        <View style={styles.photoSection}>
          <View style={styles.photoWrap}>
            {fotoLocal
              ...
              : <AvatarImage fotoPath={fotoServidor} size={120} iconSize={44} style={{ borderWidth: 2, borderColor: colors.primaryFixed }} />}
        ...
        <ExpandSection icon="verified_user" title="Verificación de Credenciales" defaultOpen>
          <View style={{ paddingTop: 12, gap: 10 }}>
            ...
            <Pressable style={[styles.uploadBtn, credencialSelec ? { borderColor: colors.primaryFixed } : null]} onPress={subirCredencial}>
        ...
        <Pressable style={styles.logoutBtn} onPress={() => navigation.navigate('landing' as never)}>
