...
import type { ContactoJugador } from '../types';
...
interface JugadorRowProps {
  contacto: ContactoJugador;
  onVerDetalle: () => void;
  onEmail: () => void;
  onWhatsApp: () => void;
}

function JugadorRow({ contacto, onVerDetalle, onEmail, onWhatsApp }: JugadorRowProps) {
  const jugador = contacto.jugador ?? ({} as any);
  const u = jugador.user ?? {};
  const nombre = `${u.nombre ?? ''} ${u.apellido ?? ''}`.trim() || 'Jugador';
  const posicion = jugador.posicion_principal ?? 'Sin posición';
  const club = jugador.club_actual ?? '—';

  const fechaRaw = contacto.created_at ?? contacto.fecha_reunion;
  const fecha = fechaRaw
    ? new Date(fechaRaw).toLocaleDateString('es-AR', { day: 'numeric', month: 'short', year: 'numeric' })
    : '—';

  const cfg = ESTADO_CFG[contacto.estado ?? 'mensaje_enviado'] ?? ESTADO_CFG.mensaje_enviado;
...
function StatMini({ label, value, subLabel, icon }: StatMiniProps) {
  return (
...
export default function MisJugadoresReclutador() {
  ...
  const [lista, setLista] = useState<ContactoJugador[]>([]);
