...
const FEET = ['Zurdo', 'Diestro', 'Ambidiestro'] as const;
const STATUS = [
...
export default function Register() {
  ...
  const [foot, setFoot] = useState<string>('Zurdo');
  const [status, setStatus] = useState<string>('libre');
  const [showModal, setShowModal] = useState(false);
  const [loading, setLoading] = useState(false);
  const [fotoUri, setFotoUri] = useState<string | null>(null); // foto local seleccionada
  ...
  const handleRegistro = async () => {
    ...
    const payload = {
      ...
      peso_kg: peso ? parseInt(peso, 10) : null,
    ...
    setLoading(true);
    try {
      const data = await auth.registrarJugador(payload as any);
      setToken(data.token);
      setUser(data.user);
