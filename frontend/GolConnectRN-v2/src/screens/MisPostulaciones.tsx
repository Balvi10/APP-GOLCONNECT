...
const TABS = ['Todas', 'Enviadas', 'En Revisión', 'Finalizadas'] as const;
...
export default function MisPostulaciones() {
  ...
  useEffect(() => {
    apiPost.misPostulaciones()
      .then((data: any) => setLista(Array.isArray(data) ? data : data.data ?? []))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);

  const filtroActual = TAB_FILTER[tab];
  const filtradas = filtroActual
    ? lista.filter((p) => p.estado === filtroActual)
    : lista;
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <Screen>
        <View style={styles.body}>
          ...
          {filtradas.map((p) => {
            const est = ESTADO_MAP[p.estado ?? 'enviada'] ?? ESTADO_MAP.enviada;
            const aviso = p.aviso ?? ({} as any);
