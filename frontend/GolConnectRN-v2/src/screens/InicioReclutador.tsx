...
import type { InicioReclutadorData, AvisoBusqueda } from '../types';
...
function StatCard({ label, value, sub, subIcon, accent }: StatCardProps) {
  return (
...
function AvisoCard({ aviso, onPress }: { aviso: AvisoBusqueda; onPress: () => void }) {
...
export default function InicioReclutador() {
  ...
  const [datos, setDatos] = useState<InicioReclutadorData | null>(null);
  const [avisos, setAvisos] = useState<AvisoBusqueda[]>([]);
